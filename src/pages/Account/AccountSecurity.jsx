...
export default function AccountSecurity() {
  ...
  const submit = async (event) => {
    ...
    setError('');
    if (newPassword !== confirmation) return setError('New-password confirmation does not match.');
    setBusy(true);
    try {
  ...
  return <div className="account-security-shell">
    <section className="account-security-card">
      ...
      {error && <div className="notice notice-danger"><AlertCircle size={18} />{error}</div>}
